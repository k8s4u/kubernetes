...
type StatefulSetLister interface {
	...
	List(selector labels.Selector) (ret []*v2alpha1.StatefulSet, err error)
...
// List lists all StatefulSets in the indexer.
func (s *statefulSetLister) List(selector labels.Selector) (ret []*v2alpha1.StatefulSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2alpha1.StatefulSet))
	})
	return ret, err
}
...
type StatefulSetNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2alpha1.StatefulSet, error)
...
// List lists all StatefulSets in the indexer for a given namespace.
func (s statefulSetNamespaceLister) List(selector labels.Selector) (ret []*v2alpha1.StatefulSet, err error) {
...
// Get retrieves the StatefulSet from the indexer for a given namespace and name.
func (s statefulSetNamespaceLister) Get(name string) (*v2alpha1.StatefulSet, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2alpha1.Resource("statefulset"), name)
	}
	return obj.(*v2alpha1.StatefulSet), nil
}
