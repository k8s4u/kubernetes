...
func TestV12StatefulSetSpecConversion(t *testing.T) {
	...
	appsv2alpha1Template := v1.PodTemplateSpec{
	...
	testcases := map[string]struct {
		...
		stsSepc2 *appsv2alpha1.StatefulSetSpec
	}{
		...
		"StatefulSetSpec Conversion 2": {
			...
			stsSepc2: &appsv2alpha1.StatefulSetSpec{
				...
				Template:            appsv2alpha1Template,
				ServiceName:         "foo",
				PodManagementPolicy: appsv2alpha1.PodManagementPolicyType("bar"),
			},
		},
	}
...
func TestV1StatefulSetStatusConversion(t *testing.T) {
	...
	*collisionCount = 1
	testcases := map[string]struct {
		...
		stsStatus2 *appsv2alpha1.StatefulSetStatus
	}{
	...
	for k, tc := range testcases {
		// apps -> appsv2alpha1
		internal1 := &appsv2alpha1.StatefulSetStatus{}
		...
		if !apiequality.Semantic.DeepEqual(internal1, tc.stsStatus2) {
			t.Errorf("%q - %q: expected\n\t%#v, got \n\t%#v", k, "from apps to appsv2alpha1", tc.stsStatus2, internal1)
		}
		...
		if !apiequality.Semantic.DeepEqual(internal2, tc.stsStatus1) {
			t.Errorf("%q - %q: expected\n\t%#v, got \n\t%#v", k, "from appsv2alpha1 to apps", tc.stsStatus1, internal2)
		}
	}
}
...
func TestV1StatefulSetUpdateStrategyConversion(t *testing.T) {
	...
	appsv2alpha1rollingUpdate := new(appsv2alpha1.RollingUpdateStatefulSetStrategy)
	appsv2alpha1rollingUpdate.Partition = partition
	...
	testcases := map[string]struct {
		...
		stsUpdateStrategy2 *appsv2alpha1.StatefulSetUpdateStrategy
	}{
		"StatefulSetUpdateStrategy Conversion 1": {
			...
			stsUpdateStrategy2: &appsv2alpha1.StatefulSetUpdateStrategy{Type: appsv2alpha1.StatefulSetUpdateStrategyType("foo")},
		},
		"StatefulSetUpdateStrategy Conversion 2": {
			...
			stsUpdateStrategy2: &appsv2alpha1.StatefulSetUpdateStrategy{
				Type:          appsv2alpha1.StatefulSetUpdateStrategyType("foo"),
				RollingUpdate: appsv2alpha1rollingUpdate,
			},
		},
	}
...
func TestV1RollingUpdateDaemonSetConversion(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		rollingUpdateDs2 *appsv2alpha1.RollingUpdateDaemonSet
	}{
		"RollingUpdateDaemonSet Conversion 2": {
			...
			rollingUpdateDs2: &appsv2alpha1.RollingUpdateDaemonSet{MaxUnavailable: &intorstr, MaxSurge: &maxSurge},
		},
	}

	for k, tc := range testcases {
		// apps -> v1
		internal1 := &appsv2alpha1.RollingUpdateDaemonSet{}
...
func TestV1DeploymentConversion(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		deployment2 *appsv2alpha1.Deployment
	}{
		"Deployment Conversion 1": {
			...
			deployment2: &appsv2alpha1.Deployment{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{appsv2alpha1.DeprecatedRollbackTo: "2"},
				},
	...
	for k, tc := range testcases {
		// apps -> v1beta2
		internal1 := &appsv2alpha1.Deployment{}
...
func TestV1DeploymentSpecConversion(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		deploymentSpec2 *appsv2alpha1.DeploymentSpec
	}{
	...
	// apps -> appsv2alpha1
	for k, tc := range testcases {
		internal := &appsv2alpha1.DeploymentSpec{}
		...
		if !apiequality.Semantic.DeepEqual(internal, tc.deploymentSpec2) {
			t.Errorf("%q - %q: expected\n\t%+v, got \n\t%+v", "apps -> appsv2alpha1", k, tc.deploymentSpec2, internal)
		}
	}
	...
	for k, tc := range testcases {
		...
		if !apiequality.Semantic.DeepEqual(internal, tc.deploymentSpec1) {
			t.Errorf("%q - %q: expected\n\t%+v, got \n\t%+v", "appsv2alpha1 -> apps", k, tc.deploymentSpec1, internal)
		}
	}
...
func TestV1DeploymentStrategyConversion(t *testing.T) {
	...
	appsv2alpha1RollingUpdate := appsv2alpha1.RollingUpdateDeployment{MaxUnavailable: &maxUnavailable, MaxSurge: &maxSurge}
	testcases := map[string]struct {
		deploymentStrategy1 *apps.DeploymentStrategy
		deploymentStrategy2 *appsv2alpha1.DeploymentStrategy
	}{
		"DeploymentStrategy Conversion 1": {
			deploymentStrategy1: &apps.DeploymentStrategy{Type: apps.DeploymentStrategyType("foo")},
			deploymentStrategy2: &appsv2alpha1.DeploymentStrategy{Type: appsv2alpha1.DeploymentStrategyType("foo")},
		},
		"DeploymentStrategy Conversion 2": {
			...
			deploymentStrategy2: &appsv2alpha1.DeploymentStrategy{Type: appsv2alpha1.DeploymentStrategyType("foo"), RollingUpdate: &appsv2alpha1RollingUpdate},
		},
	}

	for k, tc := range testcases {
		// apps -> appsv2alpha1
		internal1 := &appsv2alpha1.DeploymentStrategy{}
		if err := legacyscheme.Scheme.Convert(tc.deploymentStrategy1, internal1, nil); err != nil {
			t.Errorf("%q - %q: unexpected error: %v", k, "apps -> appsv2alpha1", err)
		}
		if !apiequality.Semantic.DeepEqual(internal1, tc.deploymentStrategy2) {
			t.Errorf("%q - %q: expected\n\t%#v, got \n\t%#v", k, "apps -> appsv2alpha1", tc.deploymentStrategy2, internal1)
		}

		// appsv2alpha1 -> apps
		internal2 := &apps.DeploymentStrategy{}
		if err := legacyscheme.Scheme.Convert(tc.deploymentStrategy2, internal2, nil); err != nil {
			t.Errorf("%q - %q: unexpected error: %v", k, "appsv2alpha1 -> apps", err)
		}
		if !apiequality.Semantic.DeepEqual(internal2, tc.deploymentStrategy1) {
			t.Errorf("%q - %q: expected\n\t%#v, got \n\t%#v", k, "appsv2alpha1 -> apps", tc.deploymentStrategy1, internal2)
		}
	}
}
...
func TestV1RollingUpdateDeploymentConversion(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		rollingUpdateDeployment2 *appsv2alpha1.RollingUpdateDeployment
	}{
		"RollingUpdateDeployment Conversion 1": {
			rollingUpdateDeployment1: &apps.RollingUpdateDeployment{},
			rollingUpdateDeployment2: &appsv2alpha1.RollingUpdateDeployment{MaxUnavailable: &nilIntStr, MaxSurge: &nilIntStr},
		},
		"RollingUpdateDeployment Conversion 2": {
			rollingUpdateDeployment1: &apps.RollingUpdateDeployment{MaxUnavailable: maxUnavailable},
			rollingUpdateDeployment2: &appsv2alpha1.RollingUpdateDeployment{MaxUnavailable: &maxUnavailable, MaxSurge: &nilIntStr},
		},
		"RollingUpdateDeployment Conversion 3": {
			rollingUpdateDeployment1: &apps.RollingUpdateDeployment{MaxSurge: maxSurge},
			rollingUpdateDeployment2: &appsv2alpha1.RollingUpdateDeployment{MaxSurge: &maxSurge, MaxUnavailable: &nilIntStr},
		},
		"RollingUpdateDeployment Conversion 4": {
			...
			rollingUpdateDeployment2: &appsv2alpha1.RollingUpdateDeployment{MaxUnavailable: &maxUnavailable, MaxSurge: &maxSurge},
		},
	}

	for k, tc := range testcases {
		// apps -> appsv2alpha1
		internal1 := &appsv2alpha1.RollingUpdateDeployment{}
		...
		if !apiequality.Semantic.DeepEqual(internal1, tc.rollingUpdateDeployment2) {
			t.Errorf("%q - %q: expected\n\t%#v, got \n\t%#v", k, "apps -> appsv2alpha1", tc.rollingUpdateDeployment2, internal1)
		}
		...
		if !apiequality.Semantic.DeepEqual(internal2, tc.rollingUpdateDeployment1) {
			t.Errorf("%q - %q: expected\n\t%#v, got \n\t%#v", k, "appsv2alpha1 -> apps", tc.rollingUpdateDeployment1, internal2)
		}
	}
}
...
func TestV1ReplicaSetSpecConversion(t *testing.T) {
	...
	*replicas = 2
	...
	testcases := map[string]struct {
		...
		replicaset2 *appsv2alpha1.ReplicaSetSpec
	}{
	...
	for k, tc := range testcases {
		// apps -> appsv2alpha1
		internal1 := &appsv2alpha1.ReplicaSetSpec{}
		...
		if !apiequality.Semantic.DeepEqual(internal1, tc.replicaset2) {
			t.Errorf("%q - %q: expected\n\t%+v, got \n\t%+v", k, "apps -> appsv2alpha1", tc.replicaset2, internal1)
		}
		...
		if !apiequality.Semantic.DeepEqual(internal2, tc.replicaset1) {
			t.Errorf("%q - %q: expected\n\t%+v, got \n\t%+v", k, "appsv2alpha1 -> apps", tc.replicaset1, internal2)
		}
	}
}
